...
void next_per(int currPos, int input_len, int curr_len)
{

    if(input_len==curr_len)
    {

        //myfile.open("11.txt",ios::app);
        //myfile
        cout<<s<<"\n";
        return;
    }
    if(!str[currPos])
        return;
    visit[currPos]=1;
    s[curr_len]=str[currPos];
    for(int i=0; i<=input_len; i++)
    {
        if(!visit[i])
            next_per(i, input_len, curr_len+1);
    }
    visit[currPos]=0;
